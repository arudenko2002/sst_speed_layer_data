...
sstRtdDag = createDag('sst-speed-layer-top-project-region-rtd')
topProjectDag = createDag('sst-speed-layer-top-project-region')

i = 0
t_build_aggregate_rtd_top_project = [None] * len(region_shards)
t_build_aggregate_top_project = [None] * len(region_shards)
t_load_aggregate_top_project = [None] * len(region_shards)
...
while i < len(region_shards):
  ...
  t_build_aggregate_rtd_top_project[i] = createBashOprTask(
    'build-aggregate-rtd-top-project-{}'.format(
      region_shards[i]
    ), 'TOPPROJECT', stage_build, start_shard, end_shard, historic_rtd_start_date, historic_rtd_end_date,  sstRtdDag)
  
  #top project historic AGG task
  t_build_aggregate_top_project[i] = createBashOprTask('build-aggregate-top-project-{}'.format(
      region_shards[i]
    ), 'TOPPROJECT', stage_build, start_shard, end_shard, historic_start_date, historic_end_date, topProjectDag)

  #top project historic LOAD task
  t_load_aggregate_top_project[i] = createDataflowOprTask('load-aggregate-top-project-{}'.format(
      region_shards[i]
    ), 'TOPPROJECT', stage_load, start_shard, end_shard, historic_start_date, historic_end_date, bt_table_name_top_project, df_startup_worker_node, topProjectDag)

  """
  if i > 0:
    t_build_aggregate_rtd_top_project[i-1] >> t_build_aggregate_rtd_top_project[i]
    t_build_aggregate_top_project[i-1] >> t_build_aggregate_top_project[i]
    t_load_aggregate_top_project[i-1] >> t_load_aggregate_top_project[i]
  """
  
  t_build_aggregate_top_project[i] >> t_load_aggregate_top_project[i]
