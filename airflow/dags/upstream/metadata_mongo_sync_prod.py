...
# DAG configuration
start_date = datetime(2017, 11, 15, 15, 30, 0, tzinfo=pytz.utc)
...
# declare DAG
dag = DAG('metadata_mongo_sync_prod',
...
for table in ingest_tables:
    ...
    mongo_drop_latest_collection = BashOperator(
        ...
        env={'IS_HOST1_MASTER': '{{ ti.xcom_pull("is_mongo_1_master") }}',
            'HOST1': '35.184.168.229',
            'HOST2': '35.188.210.175'},
        dag = dag
    )
    ...
    if table in ['artist', 'project', 'track']:
        ...
        mongo_ingest_collection >> mongo_index_collection >> mongo_rename_old_collection >> mongo_rename_new_collection 

    else:

        mongo_ingest_collection >> mongo_rename_old_collection >> mongo_rename_new_collection 
