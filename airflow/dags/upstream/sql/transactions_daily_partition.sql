...
from (select partner_report_date,
      ...
      where product_type in ('Physical', 'Bundle', 'Track', 'Video')
            ...
            and partner_report_date = cast('#partner_report_date#' as date)   
            and load_datetime > timestamp('{{ ti.xcom_pull("last_update_timestamp") }}')
      ...
      select parse_date('%Y%m%d', partner_report_date) as partner_report_date,
       ...
       where  ordered_quantity - cancelled_quantity <> 0
              and parse_date('%Y%m%d', partner_report_date) = cast('#partner_report_date#' as date)   
              and load_datetime > timestamp('{{ ti.xcom_pull("last_update_timestamp") }}')  
              and load_datetime < timestamp_add(timestamp('{{ ts }}'), interval 24 hour)
              
            ) t
inner join (select distinct sales_country_code, sst_country_code 
            from `{{ var.value.metadata_dst }}.sst_country_region`) scr
            on t.sales_country_code = scr.sales_country_code
left outer join `{{ var.value.metadata_src }}.master_account` ma on cast(t.sub_account_code as string) = cast(ma.sub_account_code as string)
...
left outer join (select distinct lp.product_id, scr.sst_country_code, lp.sap_segment_code, lp.sap_segment_name
                 from `{{ var.value.metadata_src}}.local_product` lp
                 inner join (select distinct sales_country_code, sst_country_code 
                             from `{{ var.value.metadata_dst}}.sst_country_region`) scr on lp.sales_country_code = scr.sales_country_code
                             
                 inner join `{{ var.value.metadata_dst}}.sap_segment` saps on lp.sap_segment_code = saps.sap_segment_code
