...
cast(${LABEL} as string) as label,
cast(${TRACK_GENRE} as string) as genres,
${CATALOG} as catalog,
cast(p.master_track_id as string) as master_track_id,
sum(units) as units,
sum(digital_track_units) as digital_track_units,
sum(video_stream_units) as video_stream_units,
sum(audio_stream_units) as audio_stream_units,
...
sum(video_stream_euro) as video_stream_euro,
sum(audio_stream_euro) as audio_stream_euro,
sum($ALIAS_TRANSACTION.track_adjusted_units) as track_adjusted_units,
sum(audio_stream_track_adjusted_units) as audio_stream_track_adjusted_units,
sum(video_stream_track_adjusted_units) as video_stream_track_adjusted_units,
sum(digital_track_track_adjusted_units) as digital_track_track_adjusted_units
from `$SST_SOURCE_DATASET.$TABLE_NAME_TRANSACTIONS` $ALIAS_TRANSACTION
inner join (select * from `$SST_SOURCE_DATASET.$TABLE_NAME_PRODUCT` where master_track_id is not null and master_track_id <> -1 and master_artist_id <> -1) $ALIAS_PRODUCT
on $ALIAS_TRANSACTION.product_id = $ALIAS_PRODUCT.product_id
${ADDITIONAL_JOIN}
...
cast(${TRACK_GENRE} as string) as lptd_genres,
${CATALOG} as lptd_catalog,
cast(p.master_track_id as string) as lptd_master_track_id,
sum(units) as lptd_units,
sum(euro) as lptd_euro,
sum($ALIAS_TRANSACTION.track_adjusted_units) as lptd_track_adjusted_units
from `$SST_SOURCE_DATASET.$TABLE_NAME_TRANSACTIONS` $ALIAS_TRANSACTION
inner join (select * from `$SST_SOURCE_DATASET.$TABLE_NAME_PRODUCT` where master_track_id is not null and master_track_id <> -1 and master_artist_id <> -1) $ALIAS_PRODUCT
on $ALIAS_TRANSACTION.product_id = $ALIAS_PRODUCT.product_id
${ADDITIONAL_JOIN}
