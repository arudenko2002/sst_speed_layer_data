...
lag(track_adjusted_units, 1) over window_func as lp_track_adjusted_units,
...
sum(track_adjusted_units) over (window_func ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) as rtd_track_adjusted_units,
sum(euro) over (window_func ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) as rtd_euro
from ${TBL_VAR} cp
WINDOW window_func as  (partition by master_track_id, genres, catalog,
                      period_marker,
                      territory_marker,
                      territory,
                      partner,
