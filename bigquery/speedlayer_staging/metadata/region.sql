...
from  `umg-swift.consumption_speedlayer_staging.sst_country_region`
...
from  (select distinct region_id, sst_country_code, sales_country_code
       from `umg-swift.consumption_speedlayer_staging.sst_country_region`) r
left outer join `umg-marketing.metadata.master_account` p on r.sales_country_code = p.sales_country_code
