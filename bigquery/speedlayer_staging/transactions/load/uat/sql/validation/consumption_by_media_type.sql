...
cast(sum(album_adjusted_units) - (sum(physical_album_units) + sum(digital_album_album_adjusted_units) + sum(digital_track_album_adjusted_units) + sum(stream_album_adjusted_units)) as int64) as album_adjusted_units_diff, 
...
from `umg-swift.consumption_speedlayer_staging.transactions_uat_v3`
--where _partitiontime = timestamp('2017-07-01')
group by product_id
having units_diff <> 0
