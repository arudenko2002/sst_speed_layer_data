...
from (select * from `umg-marketing.consumption.transactions_prelim_v3`
      where _partitiontime = timestamp(@datePartition)) t
left outer join `umg-marketing.metadata.master_account` ma on cast(t.sub_account_code as string) = cast(ma.sub_account_code as string)
